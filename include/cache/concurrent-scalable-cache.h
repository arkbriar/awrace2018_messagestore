...
#ifndef QUEUE_RACE_CONCURRENT_SCALABLE_CACHE_H
#define QUEUE_RACE_CONCURRENT_SCALABLE_CACHE_H
...
struct ConcurrentScalableCache {
     ...
     * returned.
     */
    bool insert(ConstAccessor& ac, const TKey& key, const TValue& value);
...
using ThreadSafeStringCache =
    ConcurrentScalableCache<LRUCacheKey, TValue, LRUCacheKey::HashCompare>;

template <class TKey, class TValue, class THash>
ConcurrentScalableCache<TKey, TValue, THash>::ConcurrentScalableCache(size_t maxSize,
                                                                      size_t numShards)
...
template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::find(ConstAccessor& ac, const TKey& key) {
    return getShard(key).find(ac, key);
}

template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::insert(ConstAccessor& ac, const TKey& key,
                                                          const TValue& value) {
    return getShard(key).insert(ac, key, value);
}

template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::insert(const TKey& key, const TValue& value) {
...
template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::snapshotKeys(std::vector<TKey>& keys) {
    for (size_t i = 0; i < m_numShards; i++) {
...
#endif  // QUEUE_RACE_CONCURRENT_SCALABLE_CACHE_H