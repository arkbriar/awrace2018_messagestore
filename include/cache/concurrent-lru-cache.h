...
#ifndef QUEUE_RACE_CONCURRENT_LRU_CACHE_H
#define QUEUE_RACE_CONCURRENT_LRU_CACHE_H
...
private:
    ...
    struct ListNode {
        ...
        explicit ListNode(const TKey& key) : m_key(key), m_prev(OutOfListMarker), m_next(nullptr) {}
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::find(ConstAccessor& ac, const TKey& key) {
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::insert(ConstAccessor& ac, const TKey& key,
                                                     const TValue& value) {
    // Insert into the CHM
    ListNode* node = new ListNode(key);
    HashMapValuePair hashMapValue(key, HashMapValue(value, node));
    if (!m_map.insert(ac.m_hashAccessor, hashMapValue)) {
        delete node;
        return false;
    }
...
template <class TKey, class TValue, class THash>
bool ConcurrentLRUCache<TKey, TValue, THash>::insert(const TKey& key, const TValue& value) {
    ConstAccessor ac;
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::snapshotKeys(std::vector<TKey>& keys) {
...
#endif  // QUEUE_RACE_CONCURRENT_LRU_CACHE_H